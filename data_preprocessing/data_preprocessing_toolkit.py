...
class DataPreprocessingToolkit(object):
    ...
    def add_weekend_stay(df):
        ...
        for idx, row in df.iterrows():
            interval = pd.date_range(row["date_from"], row["date_to"] - timedelta(hours=1), freq="D")
            if any(day in ("Friday", "Saturday") for day in interval.day_name()):
                df.at[idx, "weekend_stay"] = "True"
        return df
